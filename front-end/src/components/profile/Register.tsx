...
export default function Register(props: {
  ...
  const [inputUsernameClassName, setInputUserNameClassName] = useState<string>(
    "text-sm text-gray-base w-full mr-3 py-5 px-11 h-2 border border-gray-200 rounded mb-2 overflow-hidden shadow-lg"
  );
  const [inputEmailClassName, setInputEmailClassName] = useState<string>(
    "text-sm text-gray-base w-full mr-3 py-5 px-11 h-2 border border-gray-200 rounded mb-2 overflow-hidden shadow-lg"
  );
  const [inputPassWordClassName, setInputPassWordClassName] = useState<string>(
    "text-sm text-gray-base w-full mr-3 py-5 px-11 h-2 border border-gray-200 rounded mb-2 overflow-hidden shadow-lg"
  );
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    setInputUserNameClassName(
      "text-sm text-gray-base w-full mr-3 py-5 px-11 h-2 border border-gray-200 rounded mb-2 overflow-hidden shadow-lg"
    );
    setInputEmailClassName(
      "text-sm text-gray-base w-full mr-3 py-5 px-11 h-2 border border-gray-200 rounded mb-2 overflow-hidden shadow-lg"
    );
  ...
  if (props.loggedIn) {
    ...
    return (
      <div className="pt-28 h-96 flex flex-col items-center justify-center">
        <form noValidate autoComplete="off" onSubmit={handleSubmit}>
          ...
          <button
            className="bg-amber-500 w-full mt-4 rounded mb-2 overflow-hidden shadow-lg"
            type="submit"
          >
            Register
          </button>
