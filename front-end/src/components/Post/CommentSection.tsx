...
export default function CommentSectionNew(props: { postID: number }) {
  ...
  const dispError = () => {
    if (isError) {
      return (
        <div
          className={
            "flex items-center font-medium tracking-wide text-red-500 text-xs my-1 ml-10"
          }
        >
  ...
  return (
    <div className="p-2">
      <div>
        <p className="text-lg font-sans font-bold mt-1 text-white">
          Create a comment:
        </p>
        <form onSubmit={handleSubmit}>
          ...
          <br />
          <button
            className="p-1 bg-amber-500 w-1/6 rounded mb-1 overflow-hidden shadow-lg mt-1"
      ...
      <div>
        <p
          ...
          className="text-lg font-sans font-bold  text-white"
        >
          Comments:
        </p>
        {comments.length === 0 ? (
          <div className="flex items-center justify-center font-medium tracking-wide text-white text-s">
            No comments yet!
          </div>
        ) : (
          commentList
